-- scriptname: Clock Sync Test
-- Minimal MIDI clock send/receive test
--
-- x=1, y=8 : Play / Stop
-- x=3, y=8 : BPM 40  (INT mode only, orange)
-- x=5, y=8 : BPM 130 (INT mode only, green)
-- x=7, y=8 : INT / EXT clock toggle (dim / cyan)
--
-- INT mode : plays pattern + sends MIDI clock (0xF8) at 24ppqn
-- EXT mode : listens for 0xF8, steps pattern once per beat

local bpm = 120
local seq_running = false
local clock_ext = false

local PATTERN = {24, 27, 24, 29}  -- C1, D#1, C1, F1
local pat_idx = 0
local on_note = nil
local m_seq = nil
local m_clk = nil
