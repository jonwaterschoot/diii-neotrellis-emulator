...
local GSTAB  = {}  -- stability counter
local P_R, P_G, P_B = {}, {}, {} -- Color buffer for differential drawing

-- Global Physics Constants
local PHYS_HZ  = 10     -- ticks per second
local PHYS_INT = 1.0 / PHYS_HZ

for i = 1, PW * PH do
  GTYPE[i]=0; GSUB[i]=1; GMOVED[i]=false; GSIL[i]=false; GSTAB[i]=0
...
local function grain_note(pc, pr, gt)
  ...
  local rin  = 16 - pr  -- 0..7 (pr=9→high oct, pr=16→low oct)
...
for i = 1, 4 do
  tracks[i] = {
    start_step = 1,
    end_step = 16,
    div = (i == 4) and 2 or 4,  -- bottom track: 1/2 notes; others: 1/8
    step = 1,
    running = (i >= 3),          -- only bottom 2 tracks active on boot
...
-- ===========================================================================
-- EROSION
-- dissolve_state: 0=off, 1=slow, 2=fast  (cycles with lock button)
-- ===========================================================================
local dissolve_state = 0   -- 0=off 1=slow 2=fast
local ERODE_PROB     = 0.04 -- full-speed per-tick chance (half=0.02)
...
local clear_held = false  -- true while clearcolor button is held

-- Brightness: 0=full, 1=medium, 2=dim
local dim_lvl = 0
local dim_f   = 1.0

-- Auto-dropper: on/off, fires random short or long press per interval
local auto_drop_on   = false
local auto_drop_tick = 0
local auto_drop_slot = 1
local AUTO_DROP_INT  = 8   -- physics ticks between events
...
local function fill_res(i)
  ...
  RES[i].t = r<=50 and 1 or r<=70 and 2 or 3
  RES[i].s = math.random(2)
end
...
local function release_drop(slot)
  ...
  for pr = 3, PH do
    ...
    if not (pc==1 and pr>=3 and pr<=5) and GTYPE[i] == 0 then
      GTYPE[i]=t; GSUB[i]=s; GSTAB[i]=0
      break
    end
  end
  phys_active = true  -- dropping a grain activates physics
...
-- ===========================================================================
-- ===========================================================================
-- CONSOLIDATED CORE TICK: Physics + Bottom-row Erosion
-- ===========================================================================
local function core_tick()
        ...
        if pr < 16 then
          ...
          elseif t == 3 then -- Water: spread sideways, settle
            if GSTAB[i] < 8 then
              ...
              local si = i + dx
              if pc+dx >= 1 and pc+dx <= 8 and GTYPE[si] == 0 then
                GTYPE[si]=3; GSUB[si]=GSUB[i]; GSTAB[si]=0
                GTYPE[i]=0; GSTAB[i]=0; GMOVED[si]=true; is_dirty=true
              else
                GSTAB[i] = GSTAB[i] + 1
              end
  ...
  -- EROSION: randomly delete grains from the bottom two rows
  if dissolve_state > 0 then
    local prob = (dissolve_state == 1) and (ERODE_PROB * 0.5) or ERODE_PROB
    for pr = 15, 16 do
      local offset = (pr - 1) * 8
      for pc = 1, 8 do
        local i = offset + pc
        if GTYPE[i] > 0 and math.random() < prob then
          GTYPE[i]=0; GSTAB[i]=0; is_dirty=true
        end
      end
    end
  end
...
local function draw_grain(pc, pr)
  ...
  if grid_led_rgb then
    if dim_f < 1.0 then r=math.floor(r*dim_f); g=math.floor(g*dim_f); b=math.floor(b*dim_f) end
    grid_led_rgb(hx, hy, r, g, b)
  else grid_led(hx, hy, t>0 and math.floor(({15,9,6})[t]*dim_f) or 0) end
end
...
local ALT_COL_B = {live=8,   seq=200, scale=120}
local hum_labels = {{r=80,g=80,b=80},{r=160,g=80,b=180},{r=255,g=20,b=255}}

-- Force the P-buffer dirty for all 6 persistent ctrl button positions so they
-- are re-sent when drawing a different screen buffer (avoids the shared-cache
-- skipping issue in multi-screen mode).
local CTRL_IDXS = {
  (ALTP_R  -1)*8+ALT_PC,  (PLAYP_R-1)*8+PLAY_PC, (LOCKP_R-1)*8+LOCK_PC,
  (RESETP_R-1)*8+RESET_PC,(AUTOP_R -1)*8+AUTO_PC, (CLEARP_R-1)*8+CLEAR_PC,
}
local function dirty_ctrl_cache()
  for _,i in ipairs(CTRL_IDXS) do P_R[i]=-2 end
end
...
local function draw_ctrl_buttons()
  ...
  -- Start/Stop
  if seq_running then spx(PLAY_PC,PLAYP_R, 20,200,60)   -- level 11
  else spx(PLAY_PC,PLAYP_R, 85,20,10) end               -- level 5
  -- Lock button: ascending brightness = off/slow/fast
  if dissolve_state==0 then     spx(LOCK_PC,LOCKP_R, 68,12,12)    -- level 4: off
  elseif dissolve_state==1 then spx(LOCK_PC,LOCKP_R, 30,80,136)   -- level 8: slow
  else                          spx(LOCK_PC,LOCKP_R, 40,80,200)   -- level 11: fast
  end

  -- RESET button (soft white/grey)
  spx(RESET_PC, RESETP_R, 120, 120, 120)
  -- AUTO-DROPPER button: off=dim, on=yellow
  if auto_drop_on then spx(AUTO_PC, AUTOP_R, 200, 180, 0)
  else                 spx(AUTO_PC, AUTOP_R, 20, 20, 8) end
  -- CLEAR (clearcolor) button: bright red when held, dim red otherwise
  if clear_held then spx(CLEAR_PC, CLEARP_R, 255, 30, 30)
  else spx(CLEAR_PC, CLEARP_R, 60, 12, 12) end
end
...
local function draw_live()
  ...
  -- Highlight active seq step: orange in RGB (always visible), off/black in mono fallback
  for t=1, 4 do
    local tr = tracks[t]
    if seq_running and tr.running then
      local spc, spr = track_step_pos(t, tr.step)
      if spr>=9 and spr<=PH then
        local si = (spr - 1) * 8 + spc
        local hr, hg, hb = math.floor(255*dim_f), math.floor(140*dim_f), 0
        if P_R[si]~=hr or P_G[si]~=hg or P_B[si]~=hb then
          P_R[si], P_G[si], P_B[si] = hr, hg, hb
          local hx, hy = hw(spc, spr)
          if grid_led_rgb then grid_led_rgb(hx, hy, hr, hg, hb)
          else grid_led(hx, hy, 0) end
        end
      end
    end
  end
...
local function phys_tick()
  ...
  -- 4. Auto-dropper
  if auto_drop_on then
    auto_drop_tick = auto_drop_tick + 1
    if auto_drop_tick >= AUTO_DROP_INT then
      auto_drop_tick = 0
      local slot = auto_drop_slot
      auto_drop_slot = (auto_drop_slot % 4) + 1
      if math.random(2) == 1 then
        -- short press: one grain
        release_drop(slot)
      else
        -- long press: stream burst of same color
        local prev_on, prev_type, prev_sub = stream_on, stream_type, stream_sub
        stream_on = true; stream_type = DROP[slot].t; stream_sub = DROP[slot].s
        for _ = 1, STREAM_MAX do release_drop(slot) end
        stream_on = prev_on; stream_type = prev_type; stream_sub = prev_sub
      end
      is_dirty = true
    end
  end

  -- 5. Sequencer subdivision
  process_seq_ticks()
...
function event_grid(x, y, z)
  ...
  -- ---- AUTO-DROPPER toggle ----
  if pc==AUTO_PC and pr==AUTOP_R then
    if z==1 then
      auto_drop_on = not auto_drop_on
      auto_drop_tick = 0
      if auto_drop_on then phys_active = true end
      is_dirty = true
    end
    return
  end

  -- ---- CLEAR button: hold + tap a color to delete all grains of that color ----
  if pc==CLEAR_PC and pr==CLEARP_R then
    if z==1 then clear_held = true else clear_held = false end
    is_dirty = true
    return
  end

  -- ---- Lock button: toggle dissolve on/off ----
  ...
  if pr==1 then
    if z==0 then return end
    if clear_held then
      -- CLEARCOLOR: delete all grains matching the tapped reservoir slot's color
      local t, s = RES[pc].t, RES[pc].s
      if t > 0 then
        for i = 1, 128 do
          if GTYPE[i] == t and GSUB[i] == s then
            GTYPE[i] = 0; GSTAB[i] = 0; GSIL[i] = false
          end
        end
        is_dirty = true
      end
  ...
  if scr=="scale" or cur_screen=="scale" then
    ...
    if pr==6 and pc<=6 then
      scale_mode=pc; gen_scale(); is_dirty=true; return
    end
    if pr==8 and pc<=7 and KB_BLACK[pc]>=0 then
      root_note=KB_BLACK[pc]; gen_scale(); is_dirty=true; return
    end
    if pr==9 and pc<=7 then
      root_note=KB_WHITE[pc]; gen_scale(); is_dirty=true; return
    end
    if pr==11 then
    ...
    if pr==12 and pc<=4 then
      oct_base=pc+1; is_dirty=true; return
    end
    if pr==13 and pc<=3 then
      humanize_level=pc-1; is_dirty=true; return
    end
    if pr==15 and pc<=3 then
      dim_lvl = pc-1
      dim_f   = ({1.0, 0.6, 0.25})[pc]
      for i=1,128 do P_R[i]=-2 end  -- force full redraw with new brightness
      is_dirty=true; return
    end
    return
  end
  ...
  -- Tapping grains in falling/pile area plays note (or clears color if clear_held)
  if pr>=3 and pr<=16 then
    local i = (pr - 1) * 8 + pc
    if clear_held then
      -- CLEARCOLOR: delete all grains matching the tapped grain's color
      local t, s = GTYPE[i], GSUB[i]
      if t > 0 then
        for gi = 1, 128 do
          if GTYPE[gi] == t and GSUB[gi] == s then
            GTYPE[gi] = 0; GSTAB[gi] = 0; GSIL[gi] = false
          end
        end
        is_dirty = true
      end
