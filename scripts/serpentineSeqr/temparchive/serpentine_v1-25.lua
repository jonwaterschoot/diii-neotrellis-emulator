...
-- x=3, y=8: Clock source toggle (Settings only) — INT (dim) / EXT (cyan, slaves BPM to MIDI clock)
...
local clock_ext = false        -- false=INT, true=EXT (slave to incoming MIDI clock)
...
local function draw_alt()
  ...
  elseif alt_disp_timer > 0 and alt_disp_mode == "CLK" then
    if clock_ext then draw_label(8,4,"EXT",20,180,220,2)
    else draw_label(8,4,"INT",180,180,180,2) end
  else
    if hum_flash > 0 then
  ...
  if clock_ext then spx(3,8,20,180,220) else spx(3,8,30,30,30) end
  spx(5, 8, 20, 180, 60)
  spx(6, 8, 20, 80, 220)
...
function event_grid(x,y,z)
  ...
  if x==PANIC_X and y==PANIC_Y then
    ...
    if not seq_running then do_panic(); midi_tx(0xFC,0,0) else midi_tx(0xFA,0,0) end
  ...
  if alt_held then
    ...
    elseif y==3 then
      ...
      if not clock_ext then m_game:start(get_interval() / 4) end
    elseif y==8 then
      if x==3 then
...
function event_midi(b1, b2, b3)
  if b1 == 0xF8 and clock_ext and seq_running then
