...
-- x=7, y=3: Humanize level cycle — 0=off, 1–4: ±5–20 vel drift + hold probability
...
local function draw_alt()
  ...
  if humanize_level==0 then spx(7,3,10,40,45) else spx(7,3,10,50+humanize_level*40,55+humanize_level*40) end
  ...
  elseif alt_disp_timer > 0 and alt_disp_mode == "ARPE" then
    if arp_enabled then
      draw_label(8, 4, "AON", 20, 220, 80, 2)
    else
      draw_label(8, 4, "AOF", 200, 40, 40, 2)
    end
  elseif alt_disp_timer > 0 and alt_disp_mode == "HUM" then
    if humanize_level == 0 then
      draw_label(8, 4, "HU0", 60, 60, 80, 2)
    else
      draw_label(8, 4, "HU"..humanize_level, 10, 50+humanize_level*40, 55+humanize_level*40, 2)
    end
...
local function seq_tick()
  ...
  if humanize_level > 0 then
    local r = humanize_level * 5
    vel = math.max(1, math.min(127, vel + math.random(-r, r)))
    humanize_hold = math.random(4) <= humanize_level
  end
