...
--- MIDI receive callback — handles transport and clock in EXT mode.
-- 0xFA=Start, 0xFC=Stop, 0xF8=Clock (derives BPM every 24 pulses).
function event_midi(b1, b2, b3)
  if not clock_ext then return end
  if b1 == 0xFA then
    seq_running = true
    clock_pulse_count = 0; clock_last_beat_t = nil
    if alt_held then draw_alt() else draw_game() end
  ...
  elseif b1 == 0xF8 then
    ...
    if clock_pulse_count >= 24 then
      ...
      if clock_last_beat_t then
        ...
        if seq_running then m_game:start(get_interval() / 4) end
      end
