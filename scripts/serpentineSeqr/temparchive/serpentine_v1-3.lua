...
local function game_tick()
  ...
  elseif auto_mode == 1 and auto_has_target then
    local hx1,hy1 = snk(1)
    if hx1 == auto_target.x and hy1 == auto_target.y then auto_has_target = false
    else
  ...
  local sx, sy = snk(1)
  local hx, hy = wrap(sx+dir.x,1,W), wrap(sy+dir.y,1,H)
  for i=1,snake_len-1 do
    local ix, iy = snk(i)
    if ix==hx and iy==hy then
      death_phase, death_col = 1, 1
      return
    end
  end
  snake_head = (snake_head-2)%SNAKE_MAX+1
  SNAKE_X[snake_head]=hx; SNAKE_Y[snake_head]=hy
  ...
  for i=1,#fruits do
    ...
    if f.x==hx and f.y==hy then
      ...
      elseif f.kind == 4 then
        ...
        if halo_l[hkey] <= 0 then halo_s[hkey]=1; halo_l[hkey]=3
        else halo_s[hkey]=halo_s[hkey]+1; halo_l[hkey]=halo_l[hkey]-1 end
        if on_chord then for ci=1,3 do midi_note_off(chord_notes[ci]) end end
        local c_type = halo_s[hkey]
      ...
      if do_reposition then reposition_fruit(f) end
      break
    end
  end
  ...
  if alt_held then
    ...
    elseif y==1 then
      if x<=8 then
        ...
        alt_disp_mode="PAM"; alt_disp_timer=15
    ...
    elseif y==2 then
      if x<=8 then
        accent_div_idx = x
        alt_disp_mode="ACC"; alt_disp_timer=15
      else
        arp_pool_max=x-8
        alt_disp_mode="PMX"; alt_disp_timer=15
      end
    elseif y==3 then
      ...
      elseif x==5 then
        ...
        alt_disp_mode="APE"; alt_disp_timer=15
      elseif x==7 then
