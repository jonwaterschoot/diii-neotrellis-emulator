...
local function spx(x,y,r,g,b)
  ...
  -- Lift any non-black pixel so its brightest channel >= 34 (= level 2/15).
  -- This ensures intentionally dim states are visible on monochrome grids
  -- regardless of whether the host exposes grid_led_rgb or grid_led.
  if r > 0 or g > 0 or b > 0 then
    local mx = math.max(r, g, b)
    if mx < 34 then
      local sc = 34 / mx
      r = math.min(255, math.floor(r * sc))
      g = math.min(255, math.floor(g * sc))
      b = math.min(255, math.floor(b * sc))
    end
  end
