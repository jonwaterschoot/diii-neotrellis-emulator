...
-- x=3, y=8: Clock mode cycle (Settings only) — INT (dim) / OUT (orange) / EXT (cyan)
-- x=5, y=8: Base octave cycle (Settings only) — OC1–OC6
-- x=6, y=8: Octave range cycle (Settings only) — RA1–RA6
-- x=15, y=7: D-PAD UP
-- x=14, y=8: D-PAD LEFT
-- x=15, y=8: D-PAD DOWN
...
-- x=5, y=8: Base octave cycle (green) — OC1–OC6, shifts all note output
-- x=6, y=8: Octave range cycle (blue) — RA1–RA6, spreads notes across octaves
-- x=10, y=3: BPM −10
-- x=11, y=3: BPM −1
-- x=12, y=3: BPM +1
-- x=13, y=3: BPM +10
...
local clock_mode = 0          -- 0=INT 1=OUT 2=EXT
local CLOCK_LABELS = {"INT","OUT","EXT"}
local m_clock = nil
local clock_pulse_count = 0   -- counts incoming 0xF8 pulses (24 per beat)
local clock_last_beat_t = nil -- timestamp of last beat boundary
...
local function restart_clock_out()
  if not m_clock then return end
  if clock_mode == 1 then m_clock:start(60/bpm/24)
  else m_clock:stop() end
end
...
local function draw_alt()
  ...
  elseif alt_disp_timer > 0 and alt_disp_mode == "CLK" then
    local lbl = CLOCK_LABELS[clock_mode+1]
    if clock_mode==0 then draw_label(8,4,lbl,180,180,180,2)
    elseif clock_mode==1 then draw_label(8,4,lbl,220,80,20,2)
    else draw_label(8,4,lbl,20,180,220,2) end
  else
    if hum_flash > 0 then
  ...
  -- Clock mode (x=3), base octave (x=5), range (x=6) on y=8
  if clock_mode==0 then spx(3,8,30,30,30)
  elseif clock_mode==1 then spx(3,8,220,80,20)
  else spx(3,8,20,180,220) end
  spx(5, 8, 20, 180, 60)
  spx(6, 8, 20, 80, 220)
...
local function death_tick()
  ...
  if arp_pool_len > 0 then
    ...
    note = math.max(36, math.min(96, 84 - death_col*2 + math.random(-6, 6)))
  end
...
local function game_tick()
  ...
  for i=1,#fruits do
    ...
    if f.x==hx and f.y==hy then
      ...
      local eat_n = math.min(127, pool_note + 12)
      if f.kind == 1 then
      ...
      elseif f.kind == 4 then
        ...
        if on_chord then for ci=1,3 do midi_note_off(chord_notes[ci]) end end
        ...
        midi_note_on(chord_notes[1],127); midi_note_on(chord_notes[2],110); midi_note_on(chord_notes[3],95)
        on_chord = true; chord_hold_ticks = 4
      ...
      elseif f.kind == 6 then
        ...
        for ei=1,ECHO_MAX do
          if not ECHO_BUF[ei].active then
            ...
            e.vel=85; e.bounces=math.random(3,5); e.current_interval=16; e.ticks_left=e.current_interval
            break
          end
        end
      end
...
function event_grid(x,y,z)
  ...
  if x==PANIC_X and y==PANIC_Y then
    ...
    if not seq_running then
      do_panic()
      if m_clock then m_clock:stop() end
    elseif clock_mode == 1 then
      restart_clock_out()
    end
  ...
  if alt_held then
    ...
    elseif y==3 then
      ...
      if clock_mode ~= 2 then
        ...
        if clock_mode == 1 then restart_clock_out() end
      end
    elseif y==8 then
      if x==3 then
        clock_mode = (clock_mode + 1) % 3
        clock_pulse_count = 0; clock_last_beat_t = nil
        restart_clock_out()
        alt_disp_mode="CLK"; alt_disp_timer=15
      elseif x==5 then
...
--- MIDI receive callback — used for external clock sync (EXT mode).
-- Counts 0xF8 clock pulses; derives BPM once per beat (24 pulses).
-- @tparam number b1 status byte  @tparam number b2  @tparam number b3
function event_midi(b1, b2, b3)
  if b1 == 0xF8 and clock_mode == 2 and seq_running then
    clock_pulse_count = clock_pulse_count + 1
    if clock_pulse_count >= 24 then
      clock_pulse_count = 0
      local now = get_time()
      if clock_last_beat_t then
        local beat_sec = now - clock_last_beat_t
        local new_bpm = math.floor(60 / beat_sec + 0.5)
        bpm = math.max(1, math.min(240, new_bpm))
        m_game:start(get_interval() / 4)
      end
      clock_last_beat_t = now
    end
  end
end
...
m_game:start()
m_clock = metro.init(function() midi_out({0xF8}) end, 60/bpm/24)
-- m_clock starts only when clock_mode==1 (OUT) and seq is running
